...
界面采用 Material 3 Expressive 的色彩层级、圆角分组与胶囊导航，支持浅色、深色和系统主题。今日页包含进度环、周历、状态筛选和分时段添加；桌面按 Ctrl/Cmd + K 搜索。动效遵循系统的减少动态效果偏好。

快速输入保留日期、时间、优先级、标签和重复规则。重复任务每个实例只生成一次后续任务，重新打开已完成实例不会再次生成副本。连续删除可分别撤销，导入前会校验数据结构。读取失败时阻止编辑，写入失败时提示导出备份。

验证：`npm test`、`npm run build`。先运行 `npm run dev`，再在另一终端运行 `node scripts/verify-ui.mjs`，使用本机 Edge 的独立测试环境验证交互并输出截图到 `artifacts/`，不会使用日常浏览器数据。

## AI 聊天计划与动画

计划页支持连续聊天，AI 可操作真实任务列表：新增、编辑、改期、完成、恢复及删除。聊天与操作回执保存在本机 IndexedDB，每次变更可单独撤销；若任务已有后续修改，系统阻止旧响应或撤销覆盖新内容。停止请求或离开页面会取消本次操作。未配置 AI 时提供设置入口，不把本地规则冒充聊天 AI。

请求发送近期 30 条对话及当前任务给用户配置的 AI 服务，API Key 只用于认证。响应只接受已定义的 JSON 操作，先校验字段、日期和任务 ID，再一次性应用。导出的任务备份不包含聊天记录。

选中背景使用单个持续存在的指示器，在周历、筛选、导航、主题和日期选择之间平滑移动。Motion 管理组件的进入和退出，抽屉沿右侧或底部原路收回；减少动态效果时取消位移。

运行 `node scripts/verify-chat-motion.mjs` 可用独立 Edge 测试环境和模拟 AI 接口验证操作、持久化、取消请求及动画中间帧，不调用真实付费 API。结果与截图输出在 `artifacts/`。
